...
int main(void)
{
	...
	TIMER1_OC_Init();	// Timer 1 (Output Compare mode)
        ...
        TIMER5_PULSE_COUNTER_Init();
        TIMER8_PULSE_COUNTER_Init();
...
void TIMER1_OC_Init(void) //TIM1_CH3 CC 
{
	...
	TIM1->CCER &= ~(1<<(4*(3-1)+1));  // CC3P: OCPolarity_Active Low           dd                    
...
/*
void TIM5_IRQHandler(void)  	// sw3누를떄마다 인터럽트 발생
{
	TIM5->SR &= ~(1<<2);	// Interrupt flag Clear
