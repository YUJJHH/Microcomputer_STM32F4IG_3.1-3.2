...
{     	// ADC1: PA1(pin 41)
	...
	ADC1->CR2 |=  (2<<28);	//폴링	// EXTEN[1:0]: ADC_ExternalTrigConvEdge_Enable(Falling Edge)
        ADC1->CR2 |=  (1<<28);	//라이징	// EXTEN[1:0]: ADC_ExternalTrigConvEdge_Enable(Falling Edge)    밑에 인터럽트도 라이징으로 변경

	//*****************//
	ADC1->CR2 |= (0x0F<<24);	// EXTSEL[3:0]: ADC_ExternalTrig (EXTI11)       이거 설정해서 따로 버튼 설정  안해도 되는듯 
