...
// TIM4 Channel 3 : PWM 1 mode
	...
	TIM4->CR1 &= ~(3<<5); 	// CMS(Center-aligned mode Sel)=00 : Edge-aligned mode(reset state)          //주기 10ms
        
 //       	TIM4->CR1 |= (3<<5); 	// CMS(Center-aligned mode Sel)=11  센터  : 주기 20ms 됨   엣지보다 센터가 주기 2배 됨
...
//        TIM4->CCER	&= ~(1<<9);	// CC3P=0: CC3 Output Polarity (OCPolarity_High : OC3으로 반전없이 출력)
            TIM4->CCER	|= (1<<9);	// CC3P=1: CC3 Output Polarity (OCPolarity_High : OC3으로 반전해서 출력)
