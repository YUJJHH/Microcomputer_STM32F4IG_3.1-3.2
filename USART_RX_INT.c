		...
		switch(KEY_Scan())
		{
			case SW0_PUSH : 		//SW0
				...
				SerialSendString("HELLO! "); // USART1을 통해 PC로 “HELLO! ” 문자열을 전송
                       ...
                       //         SerialSendChar('H');  // 한 문자 보낼 떄
                          //       SerialSendChar(0x31); // 한 문자 보낼 떄
			break;
...
{       
	...
	//	SerialSendChar(ch); 		// PC에 수신된 문자를 Echo  : 필수는 아님 ,확인용
...
void SerialSendChar(uint8_t Ch) // 1문자 보내기 함수        ,    중요
{
	// USART_SR_TXE(1<<7)=0?, TX Buffer NOT Empty? 
	// TX buffer Empty되지 않으면 계속 대기(송신 가능한 상태까지 대기)
        while((USART1->SR & USART_SR_TXE) == RESET);   // 폴링방법   ,,  다량의 데이터: 인터럽트
