...
int main(void)
{
 ...
 //              ADC1->CR2 |= ADC_CR2_SWSTART; 	// 0x40000000 (1<<30)  시작 명령   : while 문 밖에 있으서 한번만 측정함  ,  여기서 연속모드로 해야함
	while(1)
	{
         ...
         //       Voltage = ADC_Value * (3.3 * 100) / 4095;   // 3.3V: 4095 =  Volatge : ADC_Value       원래는 볼티지를 플로팅으로 선언해야됨
                                                            // 100:  소수점아래 두자리까지 표시하기 위한 값  
            
                Voltage = ADC_Value * (3.3 * 100) / 255;   // 3.3V: 255 =  Volatge : ADC_Value     255 : 8비트           
                                                            // 1000:  소수점아래 세자리까지 표시하기 위한 값  
                //원래는 볼티지를 플로팅으로.....
                ...
                //연습
                sprintf(str,"%4d",Voltage);	// ADC result : 12bits (0~4095)
        	LCD_DisplayText(5,6,str);
                
                sprintf(str,"%2.2f", ADC_Value * (3.3 * 100) / 255);	// ADC result : 12bits (0~4095)
        	LCD_DisplayText(6,6,str);
 	}
}
...
{   	// ADC1: PA1(pin 41)
	...
	ADC->CCR &= ~(0x1F<<0);		// MULTI[4:0]: ADC_Mode_Independent  서로 독립
...
//	ADC1->CR1 &= ~(3<<24);		// RES[1:0]=0b00 : 12bit Resolution
        ADC1->CR1 |= (2<<24);		// RES[1:0]=0b00 : 8bit Resolution
        
	ADC1->CR1 &= ~(1<<8);		// SCAN=0 : ADC_ScanCovMode Disable  스캔x

	ADC1->CR2 &= ~(1<<1);		// CONT=0: ADC_Continuous ConvMode Disable  연속x
//        ADC1->CR2 |= (1<<1);		// CONT=1: ADC_Continuous ConvMode Disable  연속 사용
        
	ADC1->CR2 &= ~(3<<28);		// EXTEN[1:0]=0b00: ADC_ExternalTrigConvEdge_None  
	ADC1->CR2 &= ~(1<<11);		// ALIGN=0: ADC_DataAlign_Right
	ADC1->CR2 |= (1<<10);		// EOCS=1: The EOC bit is set at the end of each regular conversion   개별적으로
