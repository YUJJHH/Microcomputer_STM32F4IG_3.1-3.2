//////////////////////////////////////////////////////////////////////
// USART: BT 연결용 프로그램 (Android용)
// UART4: TX pin: PC10, RX pin: PC11, BT-RST pin:PC13 
	...
	while(1)
	{
		switch(KEY_Scan())
		{
			...
			case SW1_PUSH : 		//SW1
				...
				// BT 모듈을 외부 BT기기(핸드폰)에서 찾게하기위한 명령
				// 'Standby' --> 'Pending'
				SerialSendChar(0x0D);// '\r' carriage return
				LCD_DisplayText(4,0,"                        ");
			...
			case SW2_PUSH : 		//SW2
				...
				SerialSendString("AT+BTCANCEL"); // 연결해지
				SerialSendChar(0x0D);// '\r' carriage return
				LCD_DisplayText(4,0,"                        ");
			...
			case SW3_PUSH : 		//SW3
				...
				SerialSendString("AT+BTNAME=THU10"); 
				// BT 모듈 이름 변경
				SerialSendChar(0x0D);// '\r' carriage return
				LCD_DisplayText(4,0,"                        ");
				LCD_DisplayText(4,0,"AT+BTNAME=THU10");
				LCD_DisplayText(6,0,"                               ");
				no= 0;
			break;
			case SW4_PUSH : 		//SW4
				...
				SerialSendString("ATZ");         //리셋
				// BT 명령후에 명령 인식을 위한 소프트웨어 리셋
				SerialSendChar(0x0D);// '\r' carriage return
				LCD_DisplayText(4,0,"                        ");
			...
			case SW5_PUSH : 		//SW5
				...
				SerialSendString("AT+BTINFO?0"); 
				// BT 이름 확인
				SerialSendChar(0x0D);// '\r' carriage return
				LCD_DisplayText(4,0,"                        ");
				LCD_DisplayText(4,0,"AT+BTINFO?0");
				LCD_DisplayText(6,0,"                               ");
				no= 0;
			break;
			case SW6_PUSH : 		//SW6
				...
				SerialSendString("AT+BTINFO?");  //현재상태?
				SerialSendChar(0x0D);// '\r' carriage return
				LCD_DisplayText(4,0,"                        ");
				LCD_DisplayText(4,0,"AT+BTINFO?");
				LCD_DisplayText(6,0,"                               ");
				no= 0;
			break;		
			case SW7_PUSH : 		//SW7
...
void UART4_Init(void)
{
	...
	// UART4 : RST(PC13) : GPIO
	GPIOC->MODER 	|= (1<<2*13); // GPIOC PIN13
	GPIOC->OSPEEDR 	|= (3<<2*13); // GPIOC PIN13
...
{ 
	...
	tmpreg = (integerdivider / 100) << 4;   //정수?
  
	// Find the fractional part 
	fractionaldivider = integerdivider - (100 * (tmpreg >> 4));  

	// Implement the fractional part in the register 
	if ((UART4->CR1 & USART_CR1_OVER8) != 0)	   //실수파트
	{	// 8 oversampling
