...
void TIMER1_OC_Init(void) //TIM1_CH3 CC 
{
	...
	TIM1->CCER |= (1<<(4*(3-1)+1));  // CC3P: OCPolarity_Active Low                                   극성바꿈(바꿔야 정상동작)
//	TIM1->CCER &= ~(1<<(4*(3-1)+1));  // CC3P: OCPolarity_Active High
	...
	TIM1->CR1 &= ~(1<<7);   // ARPE: Auto reload preload disable          arr이 바뀔떄는 proload 설정해야됨  지금은 x
