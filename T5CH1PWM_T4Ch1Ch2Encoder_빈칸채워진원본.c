//////////////////////////////////////////////////////////////////////               증가  ccw:up 
// Mode: PWM Mode & Original Counting(Counter) Mode                         감소  cw  :down
...
// PD13을 출력설정하고 Alternate function(TIM4_CH2)으로 사용 선언
	GPIOD->MODER 	|= (2<<26);	// 0x08000000(MODER.(27,26)=0b10), GPIOD PIN13 Output Alternate function mode 					
	GPIOD->OSPEEDR 	|= (2<<26);	// 0x08000000(OSPEEDER.(27,26)=0b11), GPIOD PIN13 Output speed (50MHz High speed)
	GPIOD->PUPDR	&= ~(3<<26); 	// GPIOD PIN13 NO Pull-up
  					// PD13 ==> TIM4_CH2
	GPIOD->AFR[1]	|= (2<<20);	// 0x00200000(AFR[1].(23~20)=0b0010): Connect TIM4 pins(PD13) to AF2(TIM3..5)
...
// Time base Mode
	...
	TIM4->CR1 &= ~(3<<5); 	// CMS(Center-aligned mode Sel)=00 (Edge-aligned mode) (reset state)
        ...
        // CR2 
	TIM4->CR2 &= ~(1<<7);	// TI1S=0(TIM4_CH1 pin is connected to TI1 input)   ,크로스되지 않음
 
	// PSC, ARR
	TIM4->PSC = 1-1;	// Prescaler=1
	TIM4->ARR = 10000;	// Auto reload  :  엔코더 count값 범위: 0~10000  ,          아까는 9999까지
...
// Output Compare Mode
	...
	TIM4->SMCR |= (1<<0);	// SMS(Slave Mode Selection)=0b001 
...
// Clock Enable : GPIOA & TIMER5
	RCC->AHB1ENR	|= (1<<0);	// GPIOA Enable
	RCC->APB1ENR 	|= (1<<0);	// TIMER5 Enable 
    						
// PA0을 출력설정하고 Alternate function(TIM2_CH1)으로 사용 선언 : PWM 출력
	GPIOA->MODER 	|= (2<<0);	// PA0 Output Alternate function mode					
	GPIOA->OSPEEDR 	|= (3<<0);	// PA0 Output speed (100MHz High speed)
	GPIOA->OTYPER	&= ~(1<<0);	// PA0 Output type push-pull (reset state)
	GPIOA->AFR[0]	|= (1<<0); 	// 0x00000002	(AFR[0].(3~0)=0b0010): Connect TIM5 pins(PA0) to AF2(TIM3..5)
					// PA0 ==> TIM5_CH1
...
// TIM2 Channel 1 : PWM 1 mode
	// Assign 'PWM Pulse Period'
	TIM2->PSC	= 8400-1;	// Prescaler 84,000,000Hz/8400 = 10,000 Hz(0.1ms)  (1~65536)
	TIM2->ARR	= 100-1;	// Auto reload  (0.1ms * 100 = 10ms : PWM Period)
    
	// Define the corresponding pin by 'Output'  
	// CCER(Capture/Compare Enable Register) : Enable "Channel 1" 
	TIM2->CCER	|= (1<<0);	// CC1E=1: OC1(TIM5_CH1) Active(Capture/Compare 1 output enable)
					// 해당핀(40번)을 통해 신호출력
	TIM2->CCER	&= ~(1<<1);	// CC1P=0: CC1 output Polarity High (OC1으로 반전없이 출력)

	// Duty Ratio 
	TIM2->CCR1	= 10;		// CCR1 value

	// 'Mode' Selection : Output mode, PWM 1
	// CCMR1(Capture/Compare Mode Register 1) : Setting the MODE of Ch1 or Ch2
	TIM2->CCMR1 	&= ~(3<<0); 	// CC1S(CC1 channel)='0b00' : Output 
	TIM2->CCMR1 	|= (1<<3); 	// OC1PE=1: Output Compare 1 preload Enable

	TIM2->CCMR1	|= (6<<4);	// OC1M: Output compare 1 mode: PWM 1 mode
	TIM2->CCMR1	|= (1<<7);	// OC1CE: Output compare 1 Clear enable
