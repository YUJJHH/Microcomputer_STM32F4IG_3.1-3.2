...
uint8_t min1, min10, hour,  hour_10, hour_1;  //unsigned char    인천 시간 변수
...
void TIMER2_Init(void)
{
	...
	TIM2->CR1 |= (1<<7);	// ARPE=1(ARR is NOT buffered) 
...
void TIMER6_Init(void)
{
	...
	TIM6->CR1 |= (1<<7);	// ARPE=1(ARR is NOT buffered) 
...
void TIM2_IRQHandler(void)      //RESET: 0
{
      TIM2->SR &= ~0x01;	// Interrupt flag Clear
       ...
       if((SW0_Flag==1) || ( click==0 )) //sw를 누르거나 초기화면 일 때
       {
         city_name1='I';
         city_name2='C';
         city_name3='N';
          ...
          EXTI->PR |= 0x4000;                 // Pending bit Clear
              ...
              if(city_name1=='I'  && city_name2=='C' && city_name3=='N' )    //인천화면 일 때만 가능
              {
                      if((TIM2->ARR == 200000 )&&(TIM6->ARR == 5000-1) && (TIM9->ARR == 1000-1 ))  //인터럽트 주기가 100ms 일 때
                      {
                                ...
                                TIM2->EGR &= ~0x01;    // UG: Update generation    
                                TIM6->EGR &= ~0x01;    // UG: Update generation    
                                TIM9->EGR &= ~0x01;    // UG: Update generation    
