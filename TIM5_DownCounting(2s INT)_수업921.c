...
//	TIM3->CR1 &= ~(1<<4);  // DIR=0(Up counter)(reset state)
	TIM5->CR1 |= (1<<4);  // DIR=1(down counter)(reset state)
    ...
    // Deciding the Period
	TIM5->PSC = 8400-1;	// Prescaler 84,000,000Hz/8400= 10000Hz (0.1ms)  (1~65536)
	TIM5->ARR = 10000;	// Auto reload  1s/ 0.1ms = 10000
...
uint8_t sec1, sec10, min1;  //unsigned char  
...
//	}
                ...
                LCD_DisplayChar(5,10,sec1+0x30);
                LCD_DisplayChar(5,9,sec10+0x30);
                LCD_DisplayChar(5,8,':');
                LCD_DisplayChar(5,7,min1+0x30);
