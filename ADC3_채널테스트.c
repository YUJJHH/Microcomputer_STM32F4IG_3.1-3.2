...
char str1[3];          //거리센서1 측정 문자열
char str2[3];          //거리센서2 측정 문자열
...
void ADC_IRQHandler(void)
{
    ...
    //     _ADC3_IN1_Init();
       ...
       if(MOVE_FLAG==1)   
       {     sprintf(str1,"%2.0f",((3.3*ADC_value[0])*5)/4095+3);  //거리 표시 0~19
     ...
     //  _ADC3_IN9_Init();      일단 12비트로 함.....................
             //온도센서2, 배포된 가변저항
       
	     sprintf(str2,"%2.0f",(3.3*ADC_value[1])/4095);  //거리표시 0~3
...
void _ADC_Init(void)
{
	...
	GPIOF->MODER |= (3<<6);	// GPIOF PIN3(PF3)         온도센서2: Analog mode
...
//	ADC->CCR |= 0x00010000;	// ADC_Prescaler_Div4 (ADC MAX Clock 36Mhz, 84Mhz(APB2)/4 = 21Mhz
      ...
      //  ADC->CCR |= (1<<23);	//Temperature sensor enable내부 온도 필요 x
...
{
	// PE11: TIM1_CH3   
	// PE11을 출력설정하고 Alternate function(TIM1_CH3)으로 사용 선언
...
void TIMER4_PWM_Init(void)  // PWM 1 모드, 주기 5초,  추종차 속도 제어
{   
// TIM4_CH2 : PB7 (165번 핀)
