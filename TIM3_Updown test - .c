...
void TIMER5_Init(void)
{
	RCC->APB1ENR |= 0x08;	// RCC_APB1ENR TIMER3 Enable

	// Setting CR1 : 0x0000 
//	TIM3->CR1 &= ~(1<<4);  // DIR=0(Up counter)(reset state)
	TIM5->CR1 |= ~(1<<4);             //DIR=0(down counter)(reset state)    다운 카운팅  |로 바꾸고 밑에 arr에서 -1을 지움
	...
	TIM5->ARR = 10000;	// Auto reload        1s/0.1ms=10000  :1초 만듬                                   //0.1ms * 10 = 1ms
//          TIM5->ARR = 100000-1;                   //10초  : 10s/0.1ms=100000  하면 안됨  arr이 65334를 넘어서  100000이 아니라 34600? 정도 로 인식함
                                                          //분모를 더 크게  84000000 hz/ 84000 로 해도 안됨 : 결론 타이머3은 절대 10초 불가 >> 32비트짜리 사용   //ㄴㄴ
                        
 //      TIM5->PSC = 60000-1;                                                    //84000000/60000= 1400
  //     TIM5->ARR = 65000-1; 
                                              //45.6s/ 700us =65000
   	// Clear the Counter    /
	TIM5->EGR |= (1<<0);	// UG(Update generation)=1    // 처음값 초기화: 쓰레기값 0으로
...
void TIM5_IRQHandler(void)  	// 1s Interrupt
{
