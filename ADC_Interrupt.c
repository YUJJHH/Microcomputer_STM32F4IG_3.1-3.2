...
void ADC_IRQHandler(void)
{
	...
	ADC1->CR2 |= ADC_CR2_SWSTART; 	// 0x40000000 (1<<30)   //연속모드가 아니니까 넣음,    연속모드면 지움
}
...
{	// ADC1: PA1(pin 41)
	...
	ADC1->CR1 |=  (1<<5);		// EOCIE=1: Interrupt enable for EOC               폴링파일에서 추가
 ...
 //       	ADC1->CR2 |=(1<<1);		// CONT=1: ADC_Continuous ConvMode Disable
	...
	NVIC->ISER[0] |= (1<<18);	// Enable ADC global Interrupt             폴링파일에서 추가
