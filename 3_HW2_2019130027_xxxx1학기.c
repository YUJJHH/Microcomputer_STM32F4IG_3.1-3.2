...
int main(void)
{
	...
	while(1)
	{
              ...
              {         switch(KEY_Scan())
                 ...
                 if(( GPIOG->ODR & 0x0001) == 0x0001) // LED0  ON이면 동작
                 {     
                          
                       if(( (!((GPIOG->ODR & 0x0004) == 0x0004))) &&(!((GPIOG->ODR & 0x0008) == 0x0008))) // LED2 와 LED3 이 OFF 일 떄
                           {
                              switch(KEY_Scan())
                                    { 
                                    case 0xFD00 :        //SW1 , 블랙커피
                                            GPIOG->ODR |= 0x0002;         //블랙커피 LED 1 on
                                                BEEP();                            //부저 1회
                                      //컵 나옴 
                                               DelayMS(1000);                    //1초후
                     ...
                     if(( (!((GPIOG->ODR & 0x0002) == 0x0002))) &&(!((GPIOG->ODR & 0x0008) == 0x0008)))   // LED1 와 LED3 이 OFF 일 떄
                            {
                                switch(KEY_Scan())                  
                                          {
                    ...
                    if(( (!((GPIOG->ODR & 0x0002) == 0x0002))) &&(!((GPIOG->ODR & 0x0004) == 0x0004)))        // LED2 와 LED1 이 OFF 일 떄
                            {
                                switch(KEY_Scan())                  
                                          {               
