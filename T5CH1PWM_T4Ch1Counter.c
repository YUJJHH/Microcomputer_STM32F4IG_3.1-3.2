...
// Time base Mode
	...
	// PSC, ARR
	TIM4->PSC = 1-1;	// Prescaler=1           보통 1분주
	TIM4->ARR = 10000-1;	// Auto reload  :  count값 범위: 0~10000    ,9999에서 일치 후 0으로 감>  10000번에 펄스발생... ? 
...
//	TIM4->CCER &= ~(1<<1);	// CC1P=0    라이징
//	TIM4->CCER &= ~(1<<3);	// CC1NP=0   
         
        TIM4->CCER |= (1<<1);	// CC1P=0     둘다           폴링, 라이징 둘다 측정하니까 두배로 됨
	TIM4->CCER |= ~(1<<3);	// CC1NP=0   
        

	// SMCR(Slave Mode Control Reg.) : External Clock Enable
//	TIM4->SMCR |= (5<<4);	// TS(Trigger Selection)=0b101 :TI1FP1(Filtered Timer Input 1 출력신호)  라이징
        TIM4->SMCR |= (4<<4);	// TS(Trigger Selection)=0b101 :TI1FP1(Filtered Timer Input 1 출력신호)   둘다
