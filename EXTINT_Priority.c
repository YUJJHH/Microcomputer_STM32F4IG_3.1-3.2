...
//	NVIC_SetPriority(23, 0x0E);  // EXTI8               //밑밑밑줄하고 기능 동일함
//	NVIC_SetPriority(40, 0x0F);  // EXTI15

	NVIC->IP[23]= 0xF0;  // EXTI9_5 ,  High Priority     EXTI8번에 대한   15
	NVIC->IP[40]= 0xE0;  // EXTI15_10    Low Priority    EXTI15번에 대한  14
}
...
void EXTI9_5_IRQHandler(void)		
{
	if(EXTI->PR & 0x0100)                   // EXTI8 Interrupt Pending(발생) 여부?
	{
		...
		while(ExotFlag==0) {		// SW2 누르면 while문 탈출
			C++;
                        //C값 LCD에 표시
		        DelayMS(500); 
		}    	
