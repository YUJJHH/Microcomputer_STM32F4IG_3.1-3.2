...
int main(void)
{
	...
	IWDG->PR = 0x03;	//IWDG_Prescaler_32        32k/32=1k=1000=1ms
	IWDG->RLR= 4000; //   2000=0x7D0;	//IWDG_Reload Counter Value(2sec)  1ms*4000=4s
	...
	if((RCC->CSR & 0x20000000) != RESET)// IWDG_Reset Flag
	{
		...
		RCC->CSR |= 0x01000000; // Reset Flag Clear
                LCD_DisplayText(2,0,"2.LED1 ON ");
                
	}
        else if((RCC->CSR & 0x04000000)!= RESET) // Ext sw 리셋
        {
