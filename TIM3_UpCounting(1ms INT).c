...
//	TIM3->CR1 &= ~(1<<4);  // DIR=0(Up counter)(reset state)
	TIM3->CR1 |= (1<<4);             //DIR=0(down counter)(reset state)    다운 카운팅  |로 바꾸고 밑에 arr에서 -1을 지움
    ...
    // Deciding the Period             //분주비 나눌떄 소수점없게 하도록 주의
	TIM3->PSC = 16800-1;	// Prescaler 84,000,000Hz/16800 = 5000 Hz  0.2ms (1~65536)
    //    	TIM3->PSC = 8400-1;	// Prescaler 84,000,000Hz/8400 = 10,000 Hz (0.1ms)  (1~65536)
       	TIM3->ARR = 50000;	// Auto reload        10s/0.2ms=50000  :10초 만듬                                   //0.1ms * 10 = 1ms
        
        
//	TIM3->ARR = 10000;	// Auto reload        1s/0.1ms=10000  :1초 만듬                                   //0.1ms * 10 = 1ms
//          TIM3->ARR = 100000-1;                   //10초  : 10s/0.1ms=100000  하면 안됨  arr이 65334를 넘어서  100000이 아니라 34600? 정도 로 인식함
                                                          //분모를 더 크게  84000000 hz/ 84000 로 해도 안됨 : 결론 타이머3은 절대 10초 불가 >> 32비트짜리 사용 ㄴㄴㄴㄴㄴ
                        
 //      TIM3->PSC = 60000-1;                                                    //84000000/60000= 1400
  //     TIM3->ARR = 65000-1; 
                                              //45.6s/ 700us =65000
   	// Clear the Counter    /
	TIM3->EGR |= (1<<0);	// UG(Update generation)=1    // 처음값 초기화: 쓰레기값 0으로
...
void TIM3_IRQHandler(void)  	// 1s Interrupt
{
...
//	LED_cnt++;								
// 	if(LED_cnt>=500)	// LED3 Toggle 500ms            //500이 되면 led 토글함  ...0.5초간 켜지고 꺼지고 
//	{	LED_cnt = 0;     							   
		GPIOG->ODR ^= 0x08;	   //1s 마다 led on off
//	}
}
